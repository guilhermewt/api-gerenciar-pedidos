...
public class ProductControllerIT {
	...
	void findAll_returnListOfProduct_WhenSuccessful() {
		...
		List<ProductGetRequestBody> productEntity = testRestTemplateRoleAdmin.exchange("/products/all", HttpMethod.GET,
				new HttpEntity<>(headers()), new ParameterizedTypeReference<List<ProductGetRequestBody>>() {
				}).getBody();

		Assertions.assertThat(productEntity).isNotNull();
		Assertions.assertThat(productEntity.get(0).getId()).isNotNull();
		Assertions.assertThat(productEntity.get(0)).isEqualTo(ProductCreator.createProductGetRequestBodyCreator());
	}
	...
	void findById_ReturnProduct_whenSuccessful() {
		...
		ProductGetRequestBody productEntity = testRestTemplateRoleAdmin.exchange("/products/{id}", HttpMethod.GET,
				new HttpEntity<>(headers()), ProductGetRequestBody.class, product.getId()).getBody();

		Assertions.assertThat(productEntity).isNotNull();
		Assertions.assertThat(productEntity.getId()).isNotNull();
		Assertions.assertThat(productEntity).isEqualTo(ProductCreator.createProductGetRequestBodyCreator());
	}
	...
	void save_ReturnProduct_whenSuccessful() {
		...
		ProductPostRequestBody producPostRequestBody = ProductCreator.createProductPostRequestBodyCreator();

		ResponseEntity<ProductGetRequestBody> productEntity = testRestTemplateRoleAdmin.exchange("/products/admin/{categoryId}",
				HttpMethod.POST, new HttpEntity<>(producPostRequestBody, headers()), ProductGetRequestBody.class,
	...
	void delete_RemovesProduct_whenSuccessful() {
		...
		ResponseEntity<Void> productEntity = testRestTemplateRoleAdmin.exchange("/products/admin/{id}",
				HttpMethod.DELETE, new HttpEntity<>(headers()), Void.class, ProductCreator.createProduct().getId());
		Assertions.assertThat(productEntity).isNotNull();
	...
	void delete_Returns403_WhenUserIsNotAdmin() {
		...
		ResponseEntity<Void> productEntity = testRestTemplateRoleUser.exchange("/products/admin/{id}",
				HttpMethod.DELETE, null, Void.class, ProductCreator.createProduct().getId());
